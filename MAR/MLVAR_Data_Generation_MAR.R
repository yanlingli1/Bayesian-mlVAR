...
x1=runif(N*O,-3,3) 
x2=runif(N*O,-3,3) 
x1full=matrix(x1,nrow=N,ncol=O)
x2full=matrix(x2,nrow=N,ncol=O)
missing_gen<-function(phi0,marphi11,marphi12,marphi21,marphi22){
  n=N
  nt=O
  ...
  for (i in 1:n){
    for (t in 2:nt){
      ...
      logit1[i,t]=phi0+marphi11*x1full[i,t]+marphi12*x2full[i,t]
      pr1[i,t]=exp(logit1[i,t])/(1+exp(logit1[i,t]))
      r1[i,t]=rbinom(1,1,pr1[i,t])
      
      logit2[i,t]=phi0+marphi21*x1full[i,t]+marphi22*x2full[i,t]
...
mar<-missing_gen(phi0=-1.1,marphi11=.6,marphi12=.6,marphi21=.6,marphi22=.6)
...
marmissy1<-y1
marmissy1[mar[[1]]==1]<-NA
marmissy2<-y2
marmissy2[mar[[2]]==1]<-NA
